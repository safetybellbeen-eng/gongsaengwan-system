...
/* ================= 공유 데이터 저장소 (JSONBin.io) ================= */
const JSONBIN_ID = '6a5f1f55da38895dfe79bb15';
// TODO: 정식 배포 전 Supabase Auth 및 서버 측 권한 검증으로 이전 필요 (마스터 키가 클라이언트에 노출됨)
const JSONBIN_KEY = '$2a$10$gqTKuC69jfp6lB4WNp69PegR.3.NlyHxC3/yHc4p1FO71KNCaYHGO';
...
async function placeVenueMarker(info){
  const query = info.address ? info.address : (info.name + ' 서울');
  const coords = await kakaoGeocode(query, !!info.address);
  ...
  const addrHtml = info.address
    ? `<br>${escapeHtml(info.address)}<br><button onclick="window.copyVenueAddress('${info.address.replace(/'/g,"\\'")}')" style="margin-top:6px;padding:4px 10px;border-radius:6px;border:1px solid #ccc;background:#fff;cursor:pointer;font-size:11px;">주소 복사</button>`
    : '';
  const infoWin = new kakao.maps.InfoWindow({ content: `<div style="padding:8px 12px;font-size:13px;">⚽ <b>${escapeHtml(info.name)}</b>${addrHtml}</div>` });
